...
implements ActionListener, MouseListener {
	...
	public void initDTM1() {
		sunsql.initDTM(dtm1,"select a.id 房间编号1 from roominfo " +
	...
	public void initDTM2() {
		sunsql.initDTM(dtm2,"select roomid 房间编号 from roomnum");
	}
	...
	private void saveLiveIn() {
		...
		if(chk2.isSelected())
			cluemark	= 1;
		String sqlCode[]= new String[tb2.getRowCount() * 2 + 1];	//要存数数，还要改状态，所以是两倍大小的SQL数组
		try {
			int rcss = 0;					//表格记录指针
